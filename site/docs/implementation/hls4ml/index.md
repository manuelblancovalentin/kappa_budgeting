...
| 7. Raw update | Add explicit update-application phase. | [HLS4ML-007](/docs/status/tasks?query=HLS4ML-007) | autograd/update helper headers | Stored updates can be applied with a supplied scalar alpha. |
...
| 10. Writer | Emit trainable config/function attributes. | [HLS4ML-011](/docs/status/tasks?query=HLS4ML-011) | `write_project_cpp()`, `write_parameters()` | Writer emits precomputed `loss_config_cpp`, `backward_config_cpp`, and `backward_function_cpp` without semantic inference. |
...
| 11. ENABOL bridge | Write dataset/testbench export for trainable CSIM. | [HLS4ML-012](/docs/status/tasks?query=HLS4ML-012) | `enabol/dataset.py`, bridge utilities | CSIM receives inputs, ground truth, and reference traces for comparison. |
