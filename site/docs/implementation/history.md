...
This page is a coder-facing reference for `enabol/history.py`. The module contains the `FitHistory` container used by instrumented software training loops. hls4ml CSIM outputs are handled separately by `enabol/testbench.py`.
</TBox>
...
`history.py` intentionally has no TensorFlow dependency. It receives arrays after the software training loop has already converted logs into NumPy values.
...
`FitHistory` is a thin `dict` subclass. Keys are metric names and values are arrays with one entry per logged software training step.
...
Keep `FitHistory` lightweight. Its job is to carry software-training arrays and provide simple convenience plots. Testbench parsing, CSIM statistics, and generated firmware trace analysis belong in `TestbenchData`.

## Change Log

| Task | Date | Files | Summary |
|---|---|---|---|
| [ENB-024](/docs/status/tasks?query=ENB-024) | 2026-05-23 | `enabol/history.py`, `enabol/testbench.py`, `tests/test_history.py` | Kept `FitHistory` scoped to software training and moved hls4ml CSIM trace parsing/plotting to `TestbenchData`. |
