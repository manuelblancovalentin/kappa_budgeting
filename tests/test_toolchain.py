...
from enabol.toolchain import (
    ...
    get_default_toolchain_profile_name,
    get_toolchain_profile,
    list_toolchain_profiles,
    resolve_toolchain_profile_name,
...
def test_resolve_auto_profile_from_config(tmp_path):
    config_path = tmp_path / 'toolchains.toml'
    write_config(config_path)

    assert get_default_toolchain_profile_name(config_path) == 'local-test'
    assert resolve_toolchain_profile_name('auto', config_path) == 'local-test'
    assert resolve_toolchain_profile_name('none', config_path) is None
...
def test_toolchain_environment_applies_and_restores_env(tmp_path, monkeypatch):
    ...
    with toolchain_environment('auto', backend='Vitis', config_path=config_path) as profile:
